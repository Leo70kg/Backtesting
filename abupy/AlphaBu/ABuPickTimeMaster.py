...
class AbuPickTimeMaster(object):
    ...
    def do_symbols_with_same_factors_process(cls, target_symbols, benchmark, buy_factors, sell_factors, capital,
        ...
        if n_process_pick_time <= 0:
            ...
            n_process_pick_time = ABuEnv.g_cpu_cnt      # 避免内存堵塞
        ...
        for sub_out in out:
            # 将每个子序列进程的处理结果进行合并
            sub_orders_pd, sub_action_pd, sub_all_fit_symbols_cnt, sub_mid_var_dict = sub_out
            ...
            mid_var_dict = sub_mid_var_dict if mid_var_dict is None else dict(mid_var_dict, **sub_mid_var_dict)
        ...
        if orders_pd is not None and action_pd is not None:
            ...
            # noinspection PyUnresolvedReferences
            orders_pd = orders_pd.sort_values(['buy_datetime', 'sell_datetime'])
