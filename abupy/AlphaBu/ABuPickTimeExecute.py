...
from ..CoreBu.ABuEnv import g_high_freq_data
...
def _do_pick_time_work(capital, buy_factors, sell_factors, kl_pd, benchmark, draw=False,
    ...
    if kl_pd is None or kl_pd.shape[0] == 0:
        return None, EFitError.NET_ERROR, None

    abu_pick_time_worker = AbuFuturesPickTimeWorker1
    pick_timer_worker = abu_pick_time_worker(capital, kl_pd, benchmark, buy_factors, sell_factors)
    if show_pg:
    ...
    if len(pick_timer_worker.orders) == 0:
        ...
        return None, EFitError.NO_ORDER_GEN, None
    ...
    # 需要输出的中间变量
    mid_var_dict = {'buy_factor_call': pick_timer_worker.buy_factor_call_mid_var,
                    'buy_factor_put': pick_timer_worker.buy_factor_put_mid_var,
                    'sell_factor_call': pick_timer_worker.sell_factor_call_mid_var,
                    'sell_factor_put': pick_timer_worker.sell_factor_put_mid_var
                    }

    # 最后生成list是因为tuple无法修改导致之后不能灵活处理
    return [orders_pd, action_pd], EFitError.FIT_OK, mid_var_dict
...
def do_symbols_with_same_factors(target_symbols, benchmark, buy_factors, sell_factors, capital,
        ...
        with AbuMulPidProgress(len(target_symbols), 'pick times complete', show_progress=show_progress) as progress:
            for epoch, target_symbol in enumerate(target_symbols):
                ...
                try:
                    ...
                    ret, fit_error, mid_var = _do_pick_time_work(capital, p_buy_factors, p_sell_factors, kl_pd,
                                                                 benchmark, draw=show, show_info=show,
                                                                 show_pg=(len(target_symbols) == 1 and
                                                                          show_progress))
                ...
                r_mid_var_dict[target_symbol] = mid_var

        return r_orders_pd, r_action_pd, r_all_fit_symbols_cnt, r_mid_var_dict

    orders_pd, action_pd, all_fit_symbols_cnt, mid_var_dict = _batch_symbols_with_same_factors(buy_factors, sell_factors)
    ...
    if orders_pd is not None and action_pd is not None:
        ...
        # noinspection PyUnresolvedReferences
        orders_pd = orders_pd.sort_values(['buy_datetime'])
        if apply_capital:
            # 如果非多进程环境下开始融合资金对象
            ABuTradeExecute.apply_action_to_capital(capital, action_pd, kl_pd_manager, show_progress=show_progress)

    return orders_pd, action_pd, all_fit_symbols_cnt, mid_var_dict
