...
from .ABuPickStockBase import AbuPickStockBase, reversed_result, g_futures_cn_hiscode
...
class AbuPickStockNTop(AbuPickStockBase):
    ...
    def fit_pick(self, kl_pd, target_symbol):
        ...
        if ABuEnv.g_dominant_contract:
            ...
            futures_symbol_pool = g_futures_cn_hiscode[code_lis].loc[ABuDateUtil.timestamp_to_str(
                                                                    kl_pd.index[-1])].tolist()
        ...
        if target_symbol not in symbol_pool:
            return False
