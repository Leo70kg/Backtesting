...
from ..BetaBu import ABuPositionBase, AbuVolatilityPosition
...
class AbuFactorBuyBase(six.with_metaclass(ABCMeta, AbuParamBase)):
    ...
    def make_buy_order(self, holding_cnt, day_ind=-1):
        """
        ...
        order.fit_buy_order(day_ind, holding_cnt, self)
    ...
    def read_fit_day(self, today, holding_cnt):
        """
        ...
        return self.fit_day(today, holding_cnt)

    def buy_tomorrow(self, holding_cnt):
        """
        ...
        :return 生成的交易订单AbuOrder对象
        """
        return self.make_buy_order(holding_cnt, self.today_ind)

    def buy_today(self, holding_cnt):
        """
        ...
        :return 生成的交易订单AbuOrder对象
        """
        return self.make_buy_order(holding_cnt, self.today_ind - 1)

    def check_dominant_today(self, today):
        """
        ...
        if today.date > end_date or today.date < start_date:
            return False
        else:
            return True
