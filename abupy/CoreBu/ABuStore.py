...
from ..MetricsBu import AbuMetricsFutures
...
def store_abu_result_out_put_and_plot(abu_result_tuple, show_log=True):
    """
    保存abu.run_loop_back的回测结果AbuResultTuple对象，根据当前时间戳保存来定义存储的文件夹名称，
    不同于保存在cache中，将保存在out_put文件夹中，且所有单子都使用csv进行保存，不使用hdf5进行保存
    保证外部的可读性，同时保存统计画图结果
    ...
    # 时间字符串
    time_dir = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%f")
    fn = os.path.join(ABuEnv.g_project_data_dir, base_dir, date_dir, time_dir, 'orders_{:s}.csv'.format(time_dir))
    ABuFileUtil.dump_df_csv(fn, abu_result_tuple.orders_pd)
    if show_log:
        print('save {} suc!'.format(fn))
    ABuFileUtil.ensure_dir(fn)

    fn = os.path.join(ABuEnv.g_project_data_dir, base_dir, date_dir, time_dir, 'actions_{:s}.csv'.format(time_dir))
    ABuFileUtil.dump_df_csv(fn, abu_result_tuple.action_pd)
    if show_log:
        print('save {} suc!'.format(fn))

    fn = os.path.join(ABuEnv.g_project_data_dir, base_dir, date_dir, time_dir, 'capital_{:s}.csv'.format(time_dir))
    ABuFileUtil.dump_df_csv(fn, abu_result_tuple.capital.capital_pd)
    if show_log:
        print('save {} suc!'.format(fn))

    metrics = AbuMetricsFutures(*abu_result_tuple)

    options_pd = metrics.make_options_pd()
    fn = os.path.join(ABuEnv.g_project_data_dir, base_dir, date_dir, time_dir, 'options_{:s}.csv'.format(time_dir))
    ABuFileUtil.dump_df_csv(fn, options_pd)
    if show_log:
        print('save {} suc!'.format(fn))

    upper_dir = os.path.join(ABuEnv.g_project_data_dir, base_dir, date_dir, time_dir)
    metrics.fit_metrics()
    metrics.plot_returns_cmp(only_show_returns=True, save_plot_path=upper_dir, save_result_path=upper_dir)
    metrics.plot_max_draw_down(save_plot_path=upper_dir, save_result_path=upper_dir)
    metrics.plot_each_underlying_profit(save_plot_path=upper_dir, save_result_path=upper_dir)
    metrics.plot_option_analysis(save_plot_path=upper_dir)
