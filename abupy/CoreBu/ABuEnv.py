...
class EPrivateSourceType(Enum):
    """
        私有数据源
    """
    E_PRIVATE_SOURCE_mysql = 4
    E_PRIVATE_SOURCE_JQ = 5
    E_PRIVATE_SOURCE_RQ = 6
...
g_private_data_source = EPrivateSourceType.E_PRIVATE_SOURCE_mysql
...
class MysqlCfg(Enum):
    ...
    user = 'readonly3'
    password = '1234'
    database = 'future_contracts_daily'
    database1 = 'future_contracts_minute'
...
"""切换资金统计维度，在高频数据下是否开启高频下统计，默认为False"""
g_capital_hf = False

"""切换目标操作市场，美股，A股，港股，期货，比特币等，默认美股市场"""
g_market_target = EMarketTargetType.E_MARKET_TARGET_FUTURES_CN

"""切换是否使用高频数据进行回测，目前版本支持分钟级回测，后需添加tick级回测"""
g_high_freq_data = False

"""切换是否操作获取交易标的的各时期主力合约，适用于国内期货市场，默认为False"""
g_dominant_contract = False

"""获取期货主力合约对应的每日具体月合约"""
g_rom_dir = g_project_rom_data_dir

with open(os.path.join(g_rom_dir, 'futures_cn_date.json'), "r") as load_f:
    g_futures_cn_date_dict = json.loads(load_f.read())

g_futures_cn_hiscode_df = pd.read_csv(os.path.join(g_rom_dir, 'futures_cn_hiscode.csv'), index_col=0)

"""获取期货合约对应的tick_size和合约乘数"""
with open(os.path.join(g_rom_dir, 'futures_cn_info.json'), "r") as load_f:
    g_futures_cn_info_dict = json.loads(load_f.read())

"""获取分钟k线各时间周期下的个数"""
with open(os.path.join(g_rom_dir, 'futures_kline_num.json'), "r") as load_f:
    g_futures_kline_num_dict = json.loads(load_f.read())

"""获取所有交易日日期序列，用于计算两个交易日之间的天数"""
with open(os.path.join(g_rom_dir, 'trade_date.json'), "r") as load_f:
    g_trade_date_list = json.loads(load_f.read())['trade_date']

"""获取后推一小时的对应字典"""
with open(os.path.join(g_rom_dir, 'hour_shift.json'), "r") as load_f:
    g_hour_dict = json.loads(load_f.read())
...
"""是否开启多因子标准化功能，默认开启True"""
g_factor_standardise = True
