...
from ..TradeBu.ABuCapitalFutureTest import AbuCapitalFutureDailyTest
from ..TradeBu.ABuCapitalFutureDaily import AbuCapitalFutureDaily
from ..TradeBu.ABuKLManager import AbuKLManager
from ..UtilBu import ABuDateUtil
from ..UtilBu import ABuProgress
from ..UtilBu import ABuMySQLUtil
...
def run_loop_back(read_cash, buy_factors, sell_factors, stock_picks=None, choice_symbols=None, n_folds=2,
    ...
    # 资金类初始化
    capital = AbuCapital(read_cash, benchmark, user_commission_dict=commission_dict) if ABuEnv.g_capital_hf else \
        AbuCapitalFutureDailyTest(read_cash, benchmark, user_commission_dict=commission_dict)
    
    if ABuEnv.g_market_target == ABuEnv.EMarketTargetType.E_MARKET_TARGET_FUTURES_CN:
        start_date, end_date = ABuDateUtil.get_start(n_folds, start, end)
        choice_symbols = ABuMySQLUtil.get_dominant_contract(choice_symbols, start_date, end_date)
    ...
    # 择时策略运行，多进程方式
    orders_pd, action_pd, all_fit_symbols_cnt, mid_var_dict = AbuPickTimeMaster.do_symbols_with_same_factors_process(
    ...
    abu_result = AbuResultTuple(orders_pd, action_pd, capital, benchmark, mid_var_dict)
