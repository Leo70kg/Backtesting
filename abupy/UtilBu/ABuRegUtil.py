...
def regress_xy(x, y, mode=True, zoom=False, show=False):
    ...
    if mode:
        ...
        return model, y_fit, (rad, intercept)
...
def standardise(df):
    """
    对df进行标准化
    :param df: 因子序列，dataFrame或Series格式
    :return: 标准化后的序列
    """
...
def regress_multi_xy(x, y, fit_intercept=True, standard=ABuEnv.g_factor_standardise):
    """
    使用sklearn中的linear_model进行简单多元线性拟合操作，返回model和y_fit
    :param x: DataFrame格式
    :param y: Series格式
    :param fit_intercept: 是否需要截距
    :param standard: 是否标准化
    :return: model, y_fit, 如果mode=False，返回的model=None，y_fit为
    """
    if standard:
        x = standardise(x)

    model = linear_model.LinearRegression(fit_intercept).fit(x, y)
