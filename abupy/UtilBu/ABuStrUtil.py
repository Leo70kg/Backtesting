...
def to_str(var, digit):
    """
    eg:
        in:     a=7, digit=3; a='01', digit=3
        out:    '007'; '001'
    :param var: 整数数字或者只含有整数的字符串
    :param digit: 需要转化为字符串的位数
    :return: num对应的字符串
    """
    num_digit = len(str(var))

    if num_digit < digit:
        return '0'*(digit-num_digit) + str(var)

    else:
        return str(var)


def get_num_from_str(var):
    """
    从形如‘10M’的字符串中提取字母之前的数字
    :param var:
    :return:  int格式
    """
    return int(re.search('\d+', var).group())


def get_letters(var):
    """从字符串var中取出字母，eg. 'A1905' ->  'A' """

    return re.search('\D+', var).group()
