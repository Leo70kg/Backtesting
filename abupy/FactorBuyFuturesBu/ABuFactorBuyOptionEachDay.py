...
class AbuFactorBuyOptionEachDay(AbuFactorBuyFuturesBase, BuyCallMixin):
    ...
    __slots__ = ('xd', 'cp', 'direction', 'maturity', 'premium', 'notional', 'calendar_type', 'high_ndarray',
                 ...
                 'freq_kl_pd', 'kline_num', 'close_ndarray', 'start_date', 'end_date', 'option_list',
                 'vol_df', 'check_signal', 'today_time_str_idx', 'freq_kl_pd_time_str', 'time_ndarray',
                 'option_clear_price')
    ...
    def form_signal(self, today):
        ...
        if today_ind < np.maximum(self.xd, self.xd_option) + 1:
            return False

        if self.close_ndarray[today_ind-1] < self.low_ndarray[today_ind - self.xd - 1: today_ind - 1].min() or \
                self.close_ndarray[today_ind-1] > self.high_ndarray[today_ind - self.xd - 1: today_ind - 1].max():

            return False

        if '09:15:00' in today.datetime:
            self.check_signal = today.date
            # self.option_clear_price1 = self.low_ndarray[today_ind + 1 - self.xd_option: today_ind + 1].min()
            # self.option_clear_price2 = self.high_ndarray[today_ind + 1 - self.xd_option: today_ind + 1].max()
    ...
    def fit_day(self, today, holding_object):
        ...
        if signal:
            ...
            option.clear_low_price = s - 4 * option.initial_value
            option.clear_high_price = s + 4 * option.initial_value

            # option.clear_low_price = self.option_clear_price1
            # option.clear_high_price = self.option_clear_price2
...
# noinspection PyAttributeOutsideInit
class AbuFactorBuyPutOptionEachDay(AbuFactorBuyFuturesBase, BuyPutMixin):
