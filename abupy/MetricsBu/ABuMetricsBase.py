...
class AbuMetricsBase(object):
    ...
    @classmethod
    def show_general(cls, orders_pd, action_pd, capital, benchmark, mid_var_dict, returns_cmp=False,
        ...
        :param mid_var_dict: 因子迭代步骤中产生的中间变量构成的dict对象
        ...
        """
        metrics = cls(orders_pd, action_pd, capital, benchmark, mid_var_dict,
    ...
    def __init__(self, orders_pd, action_pd, capital, benchmark, mid_var_dict, enable_stocks_full_rate_factor=False):
        """
        :param orders_pd: 回测结果生成的交易订单构成的pd.DataFrame对象
        :param action_pd: 回测结果生成的交易行为构成的pd.DataFrame对象
        :param capital: 资金类AbuCapital实例化对象
        ...
        self.mid_var_dict = mid_var_dict
        """
        ...
        self.main_symbol_list = self._get_main_symbol_list()
    ...
    @valid_check
    def plot_max_draw_down(self, save_plot_path=None, save_result_path=None, show_plot=False):
        """可视化最大回撤"""
        ...
        plt.grid(True)
        if save_plot_path:
            path = os.path.join(save_plot_path, 'max_draw_down.png')
            plt.savefig(path)
        ...
        f = open(result_path, "a") if save_result_path else save_result_path

        self.log_func('最大回撤: {:5f}'.format(down_rate), file=f)
        self.log_func('最大回撤起始时间:{}, 结束时间{}, 共回测{:3f}'.format(
            ...
            list(six.itervalues(max_draw_down))[0]), file=f)
        self.log_func(' ', file=f)
        if save_result_path:
            f.close()

    @valid_check
    def plot_each_underlying_profit(self, save_plot_path=None, save_result_path=None, show_plot=False):
        """可视化每个交易品种的盈亏"""

        deal_symbols_set = set(self.action_pd[self.action_pd['deal'] == 1].symbol)
        ...
        for main_symbol in self.main_symbol_list:
            ...
            for pos2, symbol in enumerate(deal_symbols_set):
                if main_symbol == re.search('\D+', symbol).group():
                    column_list += [symbol + '_call_holding_profit', symbol + '_put_holding_profit',
                                    symbol + '_call_clear_profit', symbol + '_put_clear_profit']

            profit_series = self.capital.capital_pd[column_list].sum(axis=1)

            self.log_func(main_symbol + '_PnL: {:.4f}%'.format(profit_series.iloc[-1] / self.capital.read_cash *
                                                               100), file=f)
            profit_series.plot()

        if save_result_path:
            f.close()
        plt.legend(self.main_symbol_list, loc='best')
        if save_plot_path:
            path = os.path.join(save_plot_path, 'each_profit.png')
            plt.savefig(path)
    ...
    def _get_main_symbol_list(self):
        """从汇总的各具体月合约得到品种合约代码"""
        ...
        for pos, deal_symbol in enumerate(deal_symbols_set):
            symbol = re.search('\D+', deal_symbol).group()
            symbol_list.append(symbol)

        main_symbol_list = list(set(symbol_list))

        return main_symbol_list
