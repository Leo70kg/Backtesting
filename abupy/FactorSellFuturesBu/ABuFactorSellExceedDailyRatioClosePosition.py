...
class AbuFactorSellExceedDailyRatioCloseCall(AbuFactorSellFuturesBase):
    ...
    def _init_self(self, **kwargs):
        ...
        self.freq_kl_pd, self.kline_num = self.get_freq_kl_pd(self.kl_pd, '1D', self.kl_pd_dict)
        self.open_ndarray = self.freq_kl_pd.open.values

        self.today_time_str = self.get_today_time_str('1D')

        self.freq_kl_pd_time_str = self.get_freq_kl_pd_time_str('1D')
    ...
    def fit_day(self, today, orders, holding_object, buy_factor):
        ...
        open_daily = self.open_ndarray[today_ind]

        chg = (today.close - open_daily) / open_daily
        ...
        if chg < -self.ratio:
            ...
            self.sell_cnt = buy_factor.holding_cnt
...
class AbuFactorSellExceedDailyRatioClosePut(AbuFactorSellFuturesBase):
