...
class AbuEuroOptionHedge(OptionHedgeBase):
    ...
    def calc_price(self, s, vol):
        ...
        t = OptionHedge.convert_day_to_year(self.maturity_days, self.calendar_type)

        price = OptionHedge.price(s, self.k, self.r, self.q, t, vol, self.cp)

        return price

    def calc_delta(self, s, vol, date):

        if date != self.today_date:
            self.today_date = date
            # calculation_date = self.get_quantlib_date(date)
            # maturity_date = self.get_quantlib_date(self.maturity_date)
            #
            # t = self.get_num_between_dates(calculation_date, maturity_date)
            self.last_days -= 1

        t = OptionHedge.convert_day_to_year(self.last_days, self.calendar_type)
