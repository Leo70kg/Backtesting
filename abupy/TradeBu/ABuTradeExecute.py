...
from ..CoreBu.ABuEnv import EMarketTargetType, g_market_target
...
def make_orders_pd(orders, kl_pd):
    ...
    for index, order in enumerate(orders):
        ...
        order_pd = pd.DataFrame(np.array([order.buy_date, order.buy_datetime, order.buy_price, order.buy_cnt,
                                          order.buy_factor, order.buy_symbol, order.buy_pos,
                                          order.buy_type_str, order.expect_direction,
                                          order.sell_type_extra, order.sell_date, order.sell_datetime, order.sell_price,
                                          order.sell_type,
                                          order.ml_features]).reshape(1, -1),
                                index=[index],
                                columns=['buy_date', 'buy_datetime', 'buy_price', 'buy_cnt', 'buy_factor', 'symbol',
                                         'buy_pos',
        ...
        mask = kl_pd[kl_pd['datetime'] == order.buy_datetime]
    ...
    ret_orders_pd.index = ret_orders_pd['buy_datetime'].tolist()
...
def transform_action(orders_pd):
    ...
    # 从order中摘出买入交易行为
    buy_actions = orders_pd.loc[:, ['buy_date', 'buy_datetime', 'buy_price', 'buy_cnt', 'symbol', 'expect_direction',
                                    'sell_price']]
    # action = buy
    buy_actions['action'] = 'buy'
    # ACTION和order都有的action使用首字母大写，内容小写区分开
    buy_actions = buy_actions.rename(columns={'buy_date': 'Date', 'buy_datetime': 'Datetime', 'buy_price': 'Price',
                                              'buy_cnt': 'Cnt',
                                              'sell_price': 'Price2', 'expect_direction': 'Direction'})
    buy_actions.index = np.arange(0, buy_actions.shape[0])

    # 从order中摘出卖出交易行为
    sell_actions = orders_pd.loc[:, ['sell_date', 'sell_datetime', 'sell_price', 'buy_cnt', 'symbol',
                                     'expect_direction', 'buy_price']]
    # action = sell
    sell_actions['action'] = 'sell'
    # action和order都有的action使用首字母大写，内容小写区分开
    sell_actions = sell_actions.rename(columns={'sell_date': 'Date', 'sell_datetime': 'Datetime', 'sell_price': 'Price',
                                                'buy_cnt': 'Cnt',
    ...
    # noinspection PyUnresolvedReferences
    action_pd = action_pd.sort_values(['Datetime', 'action'])
...
def apply_action_to_capital(capital, action_pd, kl_pd_manager, show_progress=True):
    ...
    if g_market_target == EMarketTargetType.E_MARKET_TARGET_FUTURES_CN:
        deposit_worths = capital.capital_pd.filter(regex='.*_deposit')
        holding_profit_worths = capital.capital_pd.filter(regex='.*_holding_profit')
        capital.capital_pd['deposits_blance'] = deposit_worths.sum(axis=1)
        capital.capital_pd['profits_blance'] = holding_profit_worths.sum(axis=1)

        capital.capital_pd['capital_blance'] = capital.capital_pd['deposits_blance'] + \
            capital.capital_pd['cash_blance'] + capital.capital_pd['profits_blance']
