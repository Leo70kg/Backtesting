...
class AbuCapitalFutureDaily(PickleStateMixin):
    """资金类（每日统计），针对期货回测"""
    ...
    def __init__(self, init_cash, benchmark, user_commission_dict=None):
        ...
        # self.capital_pd.set_index('trade_date', inplace=True)
        # self.capital_pd.drop_duplicates(subset=['date'], inplace=True)
        # self.capital_pd = self.capital_pd[self.capital_pd['date_week'] < 5]
        # self.capital_pd.drop('date_week', axis=1, inplace=True)
    ...
    def buy_stock(self, a_order):
        ...
        # capital_pd_slice_init = self.capital_pd[:num_index]
        # capital_pd_slice = self.capital_pd[num_index:]
        ...
        if cash >= order_cost and a_order.buy_cnt > 0:
            ...
            if has_cond1 and has_cond2:
                ...
                deposit_cnt = self.capital_pd[a_order.buy_symbol
                                              + buy_type_deposit].iloc[:num_index + 1].dropna().iloc[-1]

                holding_average_price = self.capital_pd[a_order.buy_symbol
                                                        + buy_type_holding_price].iloc[:num_index + 1].dropna().iloc[-1]
            ...
            self.capital_pd.at[time_ind, 'cash_blance'] = np.round(cash, 3)
    ...
    def sell_stock(self, a_order):
        ...
        if has_cond1 and has_cond2:
            ...
            deposit_cnt = self.capital_pd[a_order.buy_symbol + buy_type_deposit].iloc[:num_index + 1].dropna().iloc[-1]
            ...
            # cash_blance初始化init中除了第一个其它都是nan
            cash_blance = self.capital_pd['cash_blance'].dropna()
            # 截取 < time_ind的cash_blance中最后一个元素做为cash值
