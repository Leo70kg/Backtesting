...
class AbuCapitalDaily(PickleStateMixin):
    """资金类（每日统计）"""
    ...
    def __init__(self, init_cash, benchmark, user_commission_dict=None):
        ...
        def int_convert_to_timestamp(date_int):
            return pd.to_datetime(ABuDateUtil.fmt_date(date_int))
        ...
        self.capital_pd = pd.DataFrame(
            {
                ...
                'date': kl_pd['date'],
                'date_week': kl_pd['date_week'],
                'trade_date': kl_pd['date'].apply(int_convert_to_timestamp)
            },
            index=kl_pd.index)
