...
from ..CoreBu.ABuEnv import EMarketTargetType, g_futures_cn_info_dict
...
def query_commission(trade_cnt, price, symbol_name):
    """根据查找的佣金比例计算手续费"""
    commission_rate = g_futures_cn_info_dict['commission'][get_letters(symbol_name)]
    multiplier = g_futures_cn_info_dict['contract_multiplier'][get_letters(symbol_name)]

    commission = commission_rate * trade_cnt / multiplier if isinstance(commission_rate, int) else \
        commission_rate * trade_cnt * price

    return commission


def query_deposit(trade_cnt, price, symbol_name):
    """根据查找的保证金比率计算保证金"""
    deposit_ratio = g_futures_cn_info_dict['min_deposit'][get_letters(symbol_name)]
    deposit = trade_cnt * price * deposit_ratio
...
class AbuCommission(object):
    ...
    def buy_commission_func(self, a_order):
        ...
        if len(commission_list) == 1:
            ...
            record = np.array(['buy', a_order.buy_datetime, a_order.buy_symbol, commission]).reshape(1, 4)
