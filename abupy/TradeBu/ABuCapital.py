...
class AbuCapital(PickleStateMixin):
    ...
    def __init__(self, init_cash, benchmark, user_commission_dict=None):
        ...
        self.capital_pd = pd.DataFrame(
            {
                'cash_balance': np.NAN * kl_pd.shape[0],
                'stocks_balance': np.zeros(kl_pd.shape[0]),
                'atr21': kl_pd['atr21'],
                'date': kl_pd['date'],
                'datetime': kl_pd['datetime']
            },
            index=kl_pd.index)

        self.capital_pd.set_index('date', drop=False, inplace=True)

        self.capital_pd.drop_duplicates(subset=['date'], inplace=True)

        print('capital_pd')
        print(self.capital_pd)
        print('********')
        self.capital_pd['date'] = self.capital_pd['date'].astype(int)
        # cash_blance除了第一个其它都是nan
        self.capital_pd.at[
            self.capital_pd.index[0], 'cash_balance'] = self.read_cash
    ...
    def apply_k_line(self, a_k_day, kl_pd, buy_type_head):
        ...
        if a_k_day[kl_pd.name + buy_type_head + '_keep'] > 0:
            kl = kl_pd[kl_pd['datetime'] == a_k_day['datetime']]
    ...
    def apply_kl(self, action_pd, kl_pd_manager, show_progress):
        ...
        def do_apply_kl(kl_pd, buy_type_head):
            ...
            self.capital_pd['cash_balance'].fillna(method='pad', inplace=True)
            # symbol对应列持仓量对na进行处理
    ...
    def buy_stock(self, a_order):
        """
        在apply_action中每笔交易进行处理，根据买单计算cost，在时序资金对象capital_pd上修改对应cash_balance，
        以及更新对应symbol上的持仓量
        ...
        # time_ind = pd.to_datetime(a_order.buy_datetime)
        time_ind = a_order.buy_date
        # pd时间日期对象置换出对应的index number
        num_index = self.capital_pd.index.tolist().index(time_ind)
        print(num_index)
        # cash_blance初始化init中除了第一个其它都是nan
        cash_balance = self.capital_pd['cash_balance'].dropna()

        # 先截取cash_blance中 < time_ind的时间序列
        bl_ss = cash_balance[cash_balance.index <= time_ind]
        if bl_ss is None or bl_ss.shape[0] == 0:
            logging.info('bl_ss.shape[0] == 0 ' + str(a_order.buy_datetime))
            return False
        ...
        if cash >= order_cost and a_order.buy_cnt > 0:
            ...
            if has_cond1 and has_cond2:
                ...
                print(self.capital_pd[a_order.buy_symbol
                                           + buy_type_keep].iloc[:num_index + 1].dropna())
            ...
            self.capital_pd.loc[time_ind, ['cash_balance']] = np.round(cash, 3)
            # 资金时间序列中之后的cash_blance也对应开始减去order_cost
            self.capital_pd.loc[cash_balance[cash_balance.index > time_ind].index, ['cash_balance']] \
                -= order_cost
            # 在更新持仓量前，取出之前的数值
    ...
    def sell_stock(self, a_order):
        ...
        # time_ind = pd.to_datetime(a_order.sell_datetime)
        time_ind = a_order.sell_date
        # # pd时间日期对象置换出对应的index number
        num_index = self.capital_pd.index.tolist().index(time_ind)
        # 根据a_order.expect_direction确定是要更新call的持仓量还是put的持仓量
        buy_type_keep = '_call_keep' if a_order.expect_direction == 1.0 else '_put_keep'
        ...
        if has_cond1 and has_cond2:
            ...
            # cash_balance初始化init中除了第一个其它都是nan
            cash_balance = self.capital_pd['cash_balance'].dropna()
            # 截取 < time_ind的cash_balance中最后一个元素做为cash值
            cash = cash_balance[cash_balance.index <= time_ind].iloc[-1]
