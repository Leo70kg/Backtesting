...
from ..CoreBu.ABuEnv import g_market_target
        ...
        try:
            ...
            if len(klines) > 0 and len(dates) > 0:
                ...
                if BDParser.s_calc_dm:
                    ...
                    def algrithm_dsi(pse):
                        return sum(pse) / sum(abs(pse))

                    self.df['dsi'] = self.df.p_change.rolling(10).apply(func=algrithm_dsi)
                self.df.name = symbol
...
class MySqlParser(object):
    ...
    def __init__(self, symbol, df):
        """
        由于从MySQL取下的数据是规整后的dataFrame格式，但是和原作者的dataFrame相比少了几个字段，
        这里进行添加
        :param df: 请求返回的DataFrame数据
        """
        self.df = df
        if len(df) > 0:
            dates_fmt = list(map(lambda date: str(date), self.df.index))
            dates = list(map(lambda date: ABuDateUtil.date_str_to_int(date), dates_fmt))
            datetimes = self.df.index

            self.df.fillna(value=0, inplace=True)

            if g_market_target == EMarketTargetType.E_MARKET_TARGET_FUTURES_CN:
                revised_date_list, revised_week_list = ABuDateUtil.date_revise(datetimes)
                self.df['date'] = revised_date_list
                self.df['date_week'] = revised_week_list
            ...
            self.df['timestamp'] = list(map(lambda date: ABuDateUtil.time_to_timestamp(date), self.df.index))
            self.df['pre_close'] = self.df['close'].shift(1)
            self.df['pre_close'].fillna(self.df['open'], axis=0, inplace=True)
            ...
            self.df['pre_close'] = self.df['pre_close'].astype(float)

            self.df.rename(columns={'pct_chg': 'p_change'}, inplace=True)

            del_columns(self.df, ['id'])

            if MySqlParser.s_calc_dm:
                # noinspection PyTypeChecker
                self.df['p_change'] = self.df['p_change'].apply(lambda x: round(x, 8))
                self.df['log_return'] = self.df['log_return'].apply(lambda x: round(x, 8))
            self.df.name = symbol
