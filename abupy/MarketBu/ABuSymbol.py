...
def code_to_symbol(code, rs=True):
    ...
    elif code.split('.')[0].isalpha() and code.split('.')[1].isalpha():
        # 匹配国内期货市场symbol(主力连续合约)
        futures_code = code.split('.')[0]
        sub_market = EMarketSubType(code.split('.')[1])
        market = EMarketTargetType.E_MARKET_TARGET_FUTURES_CN
        return Symbol(market, sub_market, futures_code)
...
class IndexSymbol(object):
    ...
    BM_FUTURES_CN = Symbol(EMarketTargetType.E_MARKET_TARGET_FUTURES_CN, EMarketSubType.SHFE, 'AU8888')
