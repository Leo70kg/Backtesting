...
from ..MarketBu.ABuDataPrivate import MysqlApi
...
from ..CoreBu.ABuEnv import EMarketSourceType, EPrivateSourceType
...
source_dict = {EMarketSourceType.E_MARKET_SOURCE_bd.value: BDApi,
               ...
               EPrivateSourceType.E_PRIVATE_SOURCE_mysql.value: MysqlApi}
...
def kline_pd(symbol, data_mode, n_folds=2, start=None, end=None, save=True):
    ...
    try:
        ...
        if ABuEnv.g_private_data_source is None:
            ...
            # 有设置私有数据源
            source = source_dict[ABuEnv.g_private_data_source.value]
            ...
            if not issubclass(source, BaseMarket):
                raise TypeError('g_private_data_source must be a subclass of BaseMarket!!!')

            # source = source_dict[ABuEnv.g_private_data_source]
        ...
        if match:
            if data_mode == EMarketDataSplitMode.E_DATA_SPLIT_SE:
                ...
                if ABuEnv.g_high_freq_data:
                    df = df.loc[('{:s} 09:01:00'.format(start)):, :]
