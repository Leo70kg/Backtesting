...
def _kl_unique_key_alpha191(fid, end):
    """
    通过191因子中的id以及end信息生成数据存储唯一id
    :param fid: str因子序号，形如‘001’
    :param end: str日期对象 eg 2015-02-14
    :return: eg 'alpha191_001_2015'
    """
    return "alpha191_{}_{}".format(fid, end)
...
def _load_factor_csv(date_key):
    """
    针对csv存储模式，读取本地cache因子序列
    :param date_key: 因子序列索引key，针对对csv存储模式为目标csv的具体文件名
    :return:
    """
    # noinspection PyProtectedMember
    csv_dir = ABuEnv.g_project_kl_df_data_example if ABuEnv._g_enable_example_env_ipython \
...
# def dump_alpha191_df(dump_df, fid_key, date_key):
#     """
#     封装不同存储模式，根据fid_key，date_key存储dump_df因子序列
#     储存方法 fid_key->date_key->dump_df
#
#     :param dump_df: 需要存储的因子序列实体pd.DataFrame对象
#     :param fid_key: str对象，eg. '001'
#     :param date_key: str对象，eg. alpha191_001_20170214
#     :return:
#     """
#     # 默认csv模式分配工作函数
#     dump_alpha191_func = _dump_kline_csv
#     load_alpha191_key = _load_csv_key
#     load_alpha191_func = _load_kline_csv
#
#     _end = int(date_key[-8:])
#     symbol_key = 'alpha191' + fid_key
#     df_date_key = load_alpha191_key(symbol_key)
