...
from ..AlphaFactorBu import ABuCtaFactor
...
def _benchmark(df, benchmark, symbol):
    ...
    if symbol.is_futures():
        # 如果是期货市场的目标，由于使用具体合约进行回测，不需要将具体合约的金融时间序列与benchmark进行匹配
        try:
            start = benchmark.kl_pd.index[0]
            end = benchmark.kl_pd.index[-1]
            condition = (df.index <= end) & (df.index >= start) if end is not np.nan else (df.index >= start)
            return df[condition]
    ...
    kl_pd['date'] = [int((ts.date() if isinstance(ts, datetime.datetime) else ts).strftime("%Y%m%d"))
...
def _make_kl_df(symbol, data_mode, n_folds, start, end, benchmark, save):
    ...
    if df is not None:
        ...
        df.drop_duplicates(subset=['date', 'timestamp'], inplace=True)
        ...
        # if not ABuEnv._g_enable_example_env_ipython or 'atr14' not in df.columns or 'atr21' not in df.columns:
            ...
            # calc_atr(df)
            # calc_vol(df, 10)
            # calc_(df, [{'factor_name': 'dsi', 'factor_func': ABuCtaFactor.dsi, 'field': 'log_return', 'param': 8},
            #            {'factor_name': 'rsi', 'factor_func': ABuCtaFactor.rsi, 'field': 'log_return', 'param': 10}])
...
def calc_(kline_df, factor_list):
    """
    根据给定的函数计算公式，对标的的行情数据进行计算
    :param kline_df: 行情数据，dataframe格式
    :param factor_list: list格式，包含多个因子计算公式，单个因子计算公式格式为dict，包含计算公式，参数及其因子名称
    :return:
    """
    for factor in factor_list:

        field = factor.pop('field')  # 计算该因子所需行情数据中的字段名称
        param = factor.pop('param')
        factor_func = factor.pop('factor_func')
        factor_name = factor.pop('factor_name')

        kline_df[factor_name] = 0
        kline_df[factor_name] = kline_df[field].rolling(param).apply(func=factor_func)
        kline_df[factor_name].fillna(method='bfill', inplace=True)
...
def calc_vol(kline_df, freq):
    """
    为输入的kline_df金融时间序列计算波动率，计算结果直接加到kline_df的vol列
    :param kline_df: 金融时间序列pd.DataFrame对象
    :param freq: 计算波动率的周期频率
    """
    kline_df['vol'] = kline_df['log_return'].rolling(freq).std() * np.sqrt(252)
    kline_df['vol'].fillna(method='bfill', inplace=True)


def calc_post_log_ret(kline_df):
    """
    为输入的kline_df金融时间序列计算后一天的对数收益率，计算结果直接加到kline_df的post_log_return列中，为计算因子IC
    :param kline_df: 金融时间序列pd.DataFrame对象
    :return:
    """

    kline_df['post_log_return'] = kline_df['log_return'].shift(-1)
    kline_df['post_log_return'].fillna(kline_df['log_return'], axis=0, inplace=True)
